...
3. **多 Bot 上传池**：支持多个 Bot Token 共同承担上传，按上传量阈值自动轮换和冷却
4. **本地 Bot API 优先**：下载重传时可优先走本地 Bot API，减少失败后的中断和重试成本
5. **消息过滤与内容处理**：支持按消息类型过滤，并基于正则替换或丢弃文本
6. **回复关系保留**：同步时尽量恢复引用和回复链路
7. **Web UI 控制台**：通过浏览器完成配置、任务管理和日志查看
8. **版本检测**：内置版本文件和更新提示，方便确认是否有新版本
9. **便携式目录**：配置、数据库、日志和临时文件都保存在项目目录内，便于迁移和备份
