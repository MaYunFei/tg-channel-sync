...
| 重置图片/视频指纹 | 不支持 | 不支持 | 不支持 | 支持，当前覆盖图片/视频 |
| 历史批量同步 | 不适用 | 支持 | 支持 | 支持 |
...
5. `下载重传` 模式新增了“重置图片/视频指纹”选项：会在图片或视频文件尾部追加少量随机字节，以快速改变 MD5/SHA256 等基础哈希特征。
6. 该方案是“快、零依赖、弱对抗”的实现，适合绕过基础文件查重；不追求专业级去重对抗，也不保证在所有媒体处理链路中都稳定生效。
...
当前“重置图片/视频指纹”功能不再依赖额外的第三方媒体处理工具。
